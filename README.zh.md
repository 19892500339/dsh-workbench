...
| 模块 | 可视化操作 | 对应 DSH 机制 |
|---|---|---|
| 📚 RAG | **多知识库(选定文件夹为库, 可增删改)** / 语料目录 / 分块参数 / 引擎(**bm25 \| vector \| hybrid**)/ 嵌入端点 / 按库重建与检索 | 内置 BM25 + 原创向量引擎(RRF 混合)+ `workbench_search` 工具(支持 `kb_id`) |
...
| 🔄 工作流 | 内置模板(简历撰写、招聘筛选…)/ **LogicFlow 可视化编排画布(现成引擎, 左侧面板添加节点, 拖拽实时重排连线)** / 表单式节点编辑 / 手动干运行 + 逐步日志 | 确定性 dry-run 执行器;LLM 运行时执行留待 V3 |
...
| 📝 Prompt | **内置 8 套热门领域模板(软件工程/代码审查/翻译/数据分析/产品/学习/营销…)** / 模板 CRUD / `{{变量}}` 预览 / 切换生效 / **输入框内 📝 弹层: 最近使用 3 个一键切换 + 取消生效** | 宿主动态 section `workbench:active-prompt` 注入(未激活不占上下文),下一模型步骤生效 |

所有配置持久化到宿主 `settings` 服务(默认落盘 `~/.dsh/settings.yaml`),重启不丢。

> **V3 亮点(机制替换)**: 对话输入框上方新增「⚙️ 机制」选项条 —— **RAG / 工具 / 技能 / 工作流** 四个开关。每个机制默认 = **DSH 原有行为**(不注入任何内容);打开某选项 = 用工作台配置**暂时替换**该机制,宿主通过 `systemPrompt.section` 向模型动态注入机制说明(检索知识库指引 / 工具可见性 / 技能清单 / 激活工作流步骤),模型每一步都能读到;「**一键还原默认**」把所有机制恢复 DSH 默认并清空隐藏配置。
>
> **V2/V2.1 亮点**: 工作流拖拽画布(LogicFlow 全屏/实时重排)、多知识库管理、RAG 向量/混合检索(自配 OpenAI 兼容嵌入端点)、MCP 服务器启用即把工具注册进 `ctx.tools`、工具面板显示 DSH 全量工具(可隐藏/恢复)、内置热门领域提示词 + 最近使用 3 个。
